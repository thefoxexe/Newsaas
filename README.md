...
- **Worker (`npm run worker`) → un hôte qui garde un processus vivant** (Render, Fly.io, Railway, un petit VPS, un conteneur) — **confirmé pas encore déployé (2026-07-13)**, et sans lui rien ne se passe : les URLs soumises restent en `pending` pour toujours (pas d'extraction, pas de rendu), ce qui ressemble exactement à « l'app ne fait rien » — c'est très probablement tout ce que voit l'utilisateur en prod aujourd'hui. `Dockerfile.worker` (à la racine) empaquette tout ce qu'il faut (Node, Chromium via l'image Playwright officielle, ffmpeg) — construit et testé dans cette session (`docker build -f Dockerfile.worker .`, puis un vrai job d'extraction lancé dans le conteneur, passé de `pending` à `done`).

  **Option gratuite : Render.com** (free tier réel, pas un essai limité dans le temps, pas de carte bancaire requise) :
  1. Nouveau compte sur render.com → New → Web Service → connecter ce repo GitHub.
  2. Runtime : Docker, Dockerfile Path : `Dockerfile.worker`.
  3. Instance Type : **Free**.
  4. Variables d'environnement : `DATABASE_URL`, `ANTHROPIC_API_KEY`.
  5. Déployer. Render assigne une URL publique (ex. `reeljolt-worker.onrender.com`) et injecte automatiquement `PORT` — le worker écoute dessus.
  6. Ajouter `RENDER_PUBLIC_BASE_URL=https://reeljolt-worker.onrender.com/renders` comme variable, puis redéployer.
  7. **Deux limites réelles du plan gratuit à connaître** : (a) 512 Mo de RAM — Chromium + ffmpeg tiennent dedans pour un rendu à la fois, mais c'est juste ; si des rendus échouent sans raison claire, c'est le premier suspect. (b) le service s'endort après 15 min sans trafic HTTP entrant, ce qui coupe aussi la boucle de polling des jobs en attente. Contournement gratuit : un ping externe régulier sur `https://reeljolt-worker.onrender.com/healthz` (ce endpoint existe déjà dans le code) via [cron-job.org](https://cron-job.org) ou UptimeRobot, toutes les 10 minutes, pour empêcher l'endormissement.

  **Option payante mais plus simple/fiable : Railway** (~5$/mois, pas de sommeil, plus de RAM) — mêmes étapes 1-4 avec « Deploy from GitHub repo », puis Settings → Networking → Generate Domain (Railway injecte `PORT` automatiquement comme Render).
