...
describe.skipIf(!chromiumAvailable)("renderVideo (editorial, real browser + ffmpeg)", () => {
  it("produces a bit-identical MP4 across two runs of the same input", async () => {
    const root = path.join(__dirname, "..");
    const template = await loadTemplate(path.join(root, "src", "templates", "editorial"));
    ...
    const outDir = await mkdtemp(path.join(tmpdir(), "reeljolt-editorial-test-"));
