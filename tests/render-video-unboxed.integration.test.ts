...
describe.skipIf(!chromiumAvailable)("renderVideo (unboxed, real browser + ffmpeg)", () => {
  it("produces a bit-identical MP4 across two runs of the same input", async () => {
    const root = path.join(__dirname, "..");
    const template = await loadTemplate(path.join(root, "src", "templates", "unboxed"));
    ...
    const concept = AdConceptSchema.parse(
      JSON.parse(await readFile(path.join(root, "fixtures", "unboxed-concept.sample.json"), "utf-8")),
    );

    const outDir = await mkdtemp(path.join(tmpdir(), "reeljolt-unboxed-test-"));
