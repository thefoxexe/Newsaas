...
describe.skipIf(!chromiumAvailable)("renderVideo (split-duotone, real browser + ffmpeg)", () => {
  it("produces a bit-identical MP4 across two runs of the same input", async () => {
    const root = path.join(__dirname, "..");
    const template = await loadTemplate(path.join(root, "src", "templates", "split-duotone"));
    ...
    const outDir = await mkdtemp(path.join(tmpdir(), "reeljolt-split-duotone-test-"));
