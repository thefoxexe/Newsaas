...
describe.skipIf(!chromiumAvailable)("renderVideo (browser-frame, real browser + ffmpeg)", () => {
  it("produces a bit-identical MP4 across two runs of the same input", async () => {
    const root = path.join(__dirname, "..");
    const template = await loadTemplate(path.join(root, "src", "templates", "browser-frame"));
    const brandKit = BrandKitSchema.parse(
      JSON.parse(await readFile(path.join(root, "fixtures", "saas-brand-kit.sample.json"), "utf-8")),
    );
    const concept = AdConceptSchema.parse(
      JSON.parse(await readFile(path.join(root, "fixtures", "browser-frame-concept.sample.json"), "utf-8")),
    );

    const outDir = await mkdtemp(path.join(tmpdir(), "reeljolt-browser-frame-test-"));
  ...
  it("degrades gracefully (still bit-identical) when screenshotUrl is null", async () => {
    const root = path.join(__dirname, "..");
    const template = await loadTemplate(path.join(root, "src", "templates", "browser-frame"));
    const rawBrandKit = JSON.parse(
      await readFile(path.join(root, "fixtures", "saas-brand-kit.sample.json"), "utf-8"),
    ) as Record<string, unknown>;
    const brandKit = BrandKitSchema.parse({ ...rawBrandKit, screenshotUrl: null });
    const concept = AdConceptSchema.parse(
      JSON.parse(await readFile(path.join(root, "fixtures", "browser-frame-concept.sample.json"), "utf-8")),
    );

    const outDir = await mkdtemp(path.join(tmpdir(), "reeljolt-browser-frame-noshot-test-"));
